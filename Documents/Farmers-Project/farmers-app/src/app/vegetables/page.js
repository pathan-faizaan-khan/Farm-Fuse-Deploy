...
const page = () => {
  ...
  useEffect( () => {
    const Data = fetch("http://localhost:3100/favProducts")
      ...
      .then((Data) => {
        const ActualData = Data.data.vegetables;
        setItems(ActualData);
  ...
  return (
    <>
      <div className="vegetables w-[98.83vw] h-fit bg-slate-100">
        <ToastContainer
          position="top-right"
          autoClose={5000}
          hideProgressBar={false}
        ...
        <div className="max-sm:w-[100vw] flex gap-6 bg-slate-50  scrollbar-hide overflow-y-scroll p-4">
